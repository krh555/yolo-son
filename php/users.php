 ...
 */ 
  ...
  */
		...
		switch($_POST['action']){
			...
			case "login":
				...
				if( strcmp($checkpw, $row['password']) == 0 ){
					...
					$current_user = array("username" => $row['username'], "id" => $row['id']);
					//Return greeting
					echo json_encode($current_user);
				}
				else {
	...
	elseif( strcmp($_SERVER['REQUEST_METHOD'], "GET") == 0 ) {
		switch( $_GET['action'] ){
			case 'likes':
				//Get array of story ids for stories this user has liked
				$user_id = mysqli_real_escape_string($mysqli, $_GET['user_id']);
				$query = "SELECT story_id FROM userLikes WHERE user_id = '" . $user_id . "';";
				$result = $mysqli->query($query);
				$likes = array();
				while( $row = $result->fetch_array(MYSQLI_NUM) ){
					array_push($likes, $row[0]);
				}				
